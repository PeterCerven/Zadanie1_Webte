...
function insertLogin(PDO $db, $id) : int
{
    $sql = <<<SQL
    INSERT INTO logins (type, google_id) 
    VALUES (?,?)
    SQL;
    $stmt = $db->prepare($sql);
...
if (isset($_GET['code'])) {
    ...
    $_SESSION['login_id'] =  insertLogin($db, $_SESSION['google_id']);
...
echo 'alert("Boli ste prihlasený do svojho google účtu.");';
